(function() {
	...
	var width=925,height=800,padding=0;
	...
	var interface={
		buttons:{
		applications:[
			//{label:"Profitability",id:"all-app"},
            {label:"Profitability",id:"all-app"},
			{label:"Growth",id:"cat-app"},
			{label:"Financial Strength",id:"tec-app"},
			{label:"Cash",id:"cas-app"},
			{label:"Dividend Policy",id:"dec-app"},
			{label:"Dividend Quality",id:"dqec-app"},
			{label:"Forecast Dividend Growth",id:"fre-app"}
		],
		threats:[
	...
	var categories=["ratio-a","ratio-b","ratio-c","ratio-d","ratio-e"];
        //var profitabilities=["metric-a","metric-b","metric-c","metric-d","metric-e"];
	var threatCats=["adware","backdoor","botnet","code-execution","keylogger","net-worm","overflow","spyware","sql-injection"];
	var technologies=["measure-1","measure-2","measure-3","measure-4","measure-5"];
	var catbysubcat={};
	var subcats=[];
	var threatTypes=["exploit","malware"];

	//var riskColors={"-":"#0F2747",1:"#6F95AD",2:"#539997",3:"#B4CCD9",4:"#9cbb59",5:"#5F5F5F","threat":"#7D3953"};
	//var riskColors={"-":"#ccc",1:"#67A2B9",2:"#A7C439",3:"#FFC425",4:"#F6851F",5:"#CD383F","threat":"#7D3953"};
        var riskColors={"-":"#ccc",1:"#CD383F",2:"#F6851F",3:"#FFC425",4:"#A7C439",5:"#67A2B9","threat":"#7D3953"};
	var catxy={"ratio-a":[306,186],"ratio-b":[612,186],"ratio-c":[230,405],"ratio-d":[460,405],"ratio-e":[690,405]};
        //var proxy={"metric-a":[306,186],"metric-b":[612,186],"metric-c":[230,405],"metric-d":[460,405],"metric-e":[690,405]};
	var threatCatxy={
	...
	var techxy={"measure-1":[304,186],"measure-2":[612,186],"measure-3":[230,405],"measure-4":[460,405],"measure-5":[690,405]};
...
//                var prosum=d3.nest()	
//			.key(function(d) {return d.profitability;})
//			.rollup(function(d) {return d3.sum(d,function(e) {return srsly(e.bandwidth);})})
//			.map(pan.selection);
//                
//                
...
//		// {"Networking":10000, "Media":25000, "General Internet":5000, "Collaboration":100000, "Business system",50000}
//
//		var prosum1=profitabilities.map(function(c) {return prosum[c];})
//
//		// [10000,25000,5000,100000,50000]
//		var prosum2=iPack(prosum1,500,0,0)
//		// [{r:50,k:...,x:...,y:...},{r:100,...}]
//		var prosum3={};
//		profitabilities.forEach(function(c,i) {prosum3[c]=1*prosum2[i].r;})
//		// {"Networking":50, "Media":100, ...}
//
//		profitabilities.forEach(function(pro) {
//                    
//			riskScale.range(proxy[pro][1]-prosum3[pro]/2,proxy[pro][1]+prosum3[pro]/2)
//			var spro=pan.selection.filter(function(d) {return d.profitability==pro;})
//			var spro_val=spro.map(function(d) {return srsly(d.bandwidth);})
//			var pos_spro=iPack(spro_val,prosum3[pro],proxy[pro][0],proxy[pro][1]);
//			spro.forEach(function(d,j) {
//				var i=pan.dhash[d.id];
//				pan.selection[i].pos[1]=(pos_spro[j])
//				
//				//pan.selection[i].pos[1].r=pan.selection[i].pos[1].r/3
//				pan.selection[i].pos[1].tx=proxy[pro][0]
//				pan.selection[i].pos[1].ty=proxy[pro][1]
//				pan.selection[i].pos[1].max=prosum3[pro]
//			})
//		})
                //*************************************** END ***********************************************************//
	...
	pan.drawview = function(m) {
		...
		circles.on("mouseover",function(d) {
			...
			if(pan.main=="applications") {
				...
				infotip=[
					{label:"",value:d.name,title:true},
					{label:"Sales Growth:",value:d3.format(",")(+d.salesgroth)},
					{label:"COGS Growth:",value:d3.format(",")(+d.cogsgroth)},
					{label:"Gross Income Growth:",value:d3.format(",")(+d.grossincomegroth)},	
					{label:"SGA Growth:",value:d3.format(",")(+d.sgagroth)},
					{label:"Interest Expense Growth:",value:d3.format(",")(+d.interest)},	
					{label:"Pretax Income Growth:",value:d3.format(",")(+d.pretax)},
					{label:"Net Income Growth:",value:d3.format(",")(+d.netincome)},
					{label:"EPS (Basic) Growth:",value:d3.format(",")(+d.EPS)},
					{label:"EPS (Diluted) Growth:",value:d3.format(",")(+d.EPSd)},
					{label:"EBITDA Growth:",value:d3.format(",")(+d.EBITDA)}
...
pan.setInterface = function() {
		...
		d3.selectAll("#cat-app,#dqec-app").on("click",function() {
			...
			pan.drawview(pan.mode);
                ...
                d3.selectAll("#tec-app").on("click",function() {
			d3.select("#sidebar").selectAll("a").classed("active",0)
			d3.select("#view_selection").selectAll("a").classed("active",0);
			d3.select(this).classed("active",1);
			pan.mode=1;
			pan.dataLeft[2]="Profitability";
			pan.drawview(pan.mode);
		})

		d3.select("#cas-app").on("click",function() {
		...
		d3.select("#dec-app").on("click",function() {
			d3.select("#sidebar").selectAll("a").classed("active",0)
			d3.select("#view_selection").selectAll("a").classed("active",0);
			d3.select(this).classed("active",1);
			pan.mode=2
